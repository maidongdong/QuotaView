...
通过 JSON-RPC 调用 `account/rateLimits/read`，显示 5 小时和 7 天额度的剩余百分比及重置时间。剩余额度按 `100 - usedPercent` 计算。
...
应用常驻 macOS 顶部菜单栏，每 15 秒轮询一次，并监听 `account/rateLimits/updated`、`account/updated` 和 `account/login/completed`，在额度或账号变化后立即刷新。点击菜单栏额度可查看 5 小时和 7 天额度分段电量条、剩余百分比和重置时间。
